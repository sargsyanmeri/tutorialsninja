...
test.describe('Homepage Slider - Positive scenarios', ()=>{
    ...
    test('P-04 | Second slider has pagination bullets ', async({page})=>{
        await expect(page.locator('div:nth-child(5) > .swiper-pager > .swiper-button-next')).toBeVisible();
        await expect(page.locator('div:nth-child(5) > .swiper-pager > .swiper-button-prev')).toBeVisible();
  ...
  //  test('P-05 | Clicking bullet sets active bullet in second slider ', async({page})=>{
    ...
    //    await secondSlider.locator('.swiper-button-next').click();
    //    const activeBullet = secondSlider.locator('.swiper-pagination-bullet-active');
   ...
   test('P-06 | First slider shows one active image', async({page})=>{
    ...
    test('P-07 | Prev button changes active slide in first slider ', async({page})=>{
        ...
        expect(firstSlider.locator('.swiper-slide-active img')).toBeVisible;
    });
});
...
test.describe('Homepage Slider - Negative scenarios', ()=>{
    ...
    //     const firstSlider = page.locator('.swiper-viewport').first();
    //     await firstSlider.locator('.swiper-button-next').click();
    //     await expect(firstSlider.locator('.src')).toBeVisible();
    // });
